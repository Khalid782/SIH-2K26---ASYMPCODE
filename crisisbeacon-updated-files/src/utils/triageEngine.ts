...
export function analyzeDisasterReport(text: string): TriageAnalysisResult {
  ...
  const mentions = KNOWN_LOCATIONS.flatMap((loc) => [loc.name, ...(loc.aliases || [])]
    .filter((name) => lower.includes(name))
    .map((name) => ({ loc, index: lower.indexOf(name), route: /(?:from|via)\s*$/.test(lower.slice(0, lower.indexOf(name))) })));
  mentions.sort((a, b) => Number(a.route) - Number(b.route) || a.index - b.index);
  const matchedLocation = mentions.find((m) => !m.route)?.loc;
  const isKnownLocation = !!matchedLocation;
  const location = matchedLocation ? matchedLocation.fullName : 'Unspecified Location';
  const coordinates = matchedLocation ? matchedLocation.coordinates : [0, 0] as [number, number];
  ...
  const hasTrapped = /trapped|stuck|stranded|marooned|phanse|atke/.test(lower);
  if (hasTrapped) {
  ...
  // Flooding signals
  const hasFlooding = /flood|overflow|inundated|waterlogging|submerged|pani.*(?:ghar|aa gaya)|water.*(?:entering|rising)|(?:heavy|torrential) rain/.test(lower);
  if (hasFlooding) {
  ...
  // Medical signals
  const hasMedical = /injured|unconscious|ambulance needed|medical emergency|cannot breathe|casualty/.test(lower);
  if (hasMedical) {
  ...
  const hasInfrastructure = /collapsed|collapse|road damaged|tree fell|crater|toot gaya|gir gaya/.test(lower);
  ...
  // 3. Check Relevance
  const isRelevant = detectedSignals.length > 0 && !/\b(?:no flooding|no emergency|false alarm|mock drill|buy now|discount|sale)\b/.test(lower);
  ...
  if (levelMatch) {
    waterLevel = `${levelMatch[1]} ${levelMatch[3]}`;
  } else if (hasRisingWater) {
    waterLevel = 'Rising water (depth not reported)';
  }
  ...
  return {
    ...
    secondaryLocations,
    isKnownLocation,
    locationConfidence: isKnownLocation ? 70 : 0,
    coordinates,
    disasterType,
    responseNeeded,
    severity,
