...
interface AITriageConsoleProps {
  onCreateIncident: (incident: Incident) => Promise<void>;
...
export default function AITriageConsole({
  ...
  useEffect(() => setReportText(initialReport), [initialReport]);
  ...
  const buildIncident = (result: AnalysisResult, rawText: string): Incident => {
    const newId = `INC-${crypto.randomUUID()}`;
    const now = new Date();
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      try {
        ...
        res = new Response(JSON.stringify({ fallback: true, error: 'AI service unreachable. Showing rule-based fallback for operator review.' }), { headers: { 'Content-Type': 'application/json' } });
      } finally {
      ...
      } else if (!payload || res.status >= 500) {
        result = mapRuleBasedResult(reportText.trim());
        setAnalysisError('AI endpoint unavailable. Showing rule-based fallback for operator review.');
      } else {
      ...
      if (result.isRelevant && inHyderabad([result.coordinates.lat, result.coordinates.lng]) && result.locationConfidence > 0) {
        ...
        setAnalysisError('Location needs operator review. Add a specific Hyderabad locality before mapping this report.');
      }
    } catch (err: any) {
    ...
    <div className="max-w-4xl mx-auto w-full space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[11px] text-mute/80 font-mono ml-auto">
          Severity Engine &bull; NER Parser &bull; Hybrid triage
        </span>
      </div>
