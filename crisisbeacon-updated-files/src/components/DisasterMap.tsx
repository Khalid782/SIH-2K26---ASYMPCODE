...
export const DisasterMap: React.FC<DisasterMapProps> = ({
  ...
  useEffect(() => {
    ...
    incidents.filter((i) => inHyderabad(i.coordinates)).forEach((incident) => {
      ...
      popup.textContent = `${incident.location}: ${incident.severity} ${incident.disasterType}. ${incident.originalReport.slice(0, 160)}`;
  ...
  return (
    ...
    {routingReady ? <GreenRoutePanel map={map} incidents={allIncidents} /> : <p role="status" className="text-xs">Waiting for incident data. Routing unavailable.</p>}
