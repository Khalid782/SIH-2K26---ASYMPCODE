...
export function App() {
  ...
  const [incidentsReady, setIncidentsReady] = useState(!supabase);
  ...
  useEffect(() => {
    ...
    // Load all incidents so display pagination cannot remove hazards from routing.
    const fetchIncidents = async () => {
      const rows: Record<string, any>[] = [];
      let error: { message: string } | null = null;
      for (let offset = 0; ; offset += 1000) {
        const page = await client.from('incidents').select('*').order('id').range(offset, offset + 999);
      ...
      if (error) {
        ...
        setNotification('Incident data unavailable. Routing is disabled until a successful reload.');
      } else if (data) {
    ...
    const channel = client
      ...
      .on(
        ...
        (payload) => {
          const next = mapRowToIncident(payload.new as Record<string, any>);
          setIncidents((prev) => [next, ...prev.filter((i) => i.id !== next.id)]);
        }
      )
      .on(
        'postgres_changes',
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') void fetchIncidents();
        if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT' || status === 'CLOSED') setIncidentsReady(false);
      });
  ...
  const handleCreateIncidentFromTriage = async (newIncident: Incident) => {
    ...
    if (supabase) {
      ...
      if (error) throw new Error('Incident was not saved: ' + error.message);
    }
    setIncidents((prev) => [withTimestamp, ...prev.filter((i) => i.id !== withTimestamp.id)]);
    showNotification(
      `${newIncident.engineUsed || 'Triage'} → ${newIncident.id}: ${newIncident.severity} ${newIncident.disasterType} marked on map & added to feed`
    );
  };
  ...
  const handleUpdateStatus = async (incidentId: string, newStatus: VerificationStatus) => {
    if (supabase) {
      const { data, error } = await supabase.from('incidents')
        .update({ status: newStatus, verification_status: newStatus }).eq('id', incidentId).select('id');
      if (error || !data?.length) {
        showNotification('Status was not saved. Check database permissions and retry.');
        return;
      }
    }
    setIncidents((prev) => prev.map((inc) => inc.id === incidentId ? { ...inc, status: newStatus, verificationStatus: newStatus } : inc));
    setSelectedIncident((prev) => prev?.id === incidentId ? { ...prev, status: newStatus, verificationStatus: newStatus } : prev);
  ...
  return (
    <div className="h-screen w-full overflow-hidden flex flex-col text-ink dark:text-paper font-sans bg-gradient-to-br from-[#e7ddd0] via-[#ddd4c1] to-[#cfc6b3] dark:bg-gradient-to-br dark:from-ink-2 dark:via-ink dark:to-ink transition-colors duration-300">
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
        ...
        <main className="flex-1 min-w-0 min-h-0 overflow-y-auto p-3 sm:p-4 lg:p-6 space-y-4 max-w-7xl mx-auto w-full">
          <label className="md:hidden block text-xs">Workspace
            <select aria-label="Workspace" value={activeTab} onChange={(e) => setActiveTab(e.target.value)} className="block w-full mt-1 p-2 rounded border bg-white text-gray-900">
              <option value="dashboard">Situation Room</option><option value="triage">AI Triage</option>
              <option value="reports">News & Ingestion</option><option value="teams">Response Units</option>
              <option value="ngos">NGOs</option><option value="protocols">Protocols</option>
            </select>
          </label>
          {activeTab === 'triage' ? (
            <AITriageConsole
              initialReport={triageDraft}
              onCreateIncident={handleCreateIncidentFromTriage}
              onNavigateToSituationRoom={() => setActiveTab('dashboard')}
            />
          ) : activeTab === 'reports' ? (
            <FeedIngestionView onReview={(text) => { setTriageDraft(text); setActiveTab('triage'); }} />
          ) : activeTab === 'teams' ? (
            <ResponseUnitsView />
          ) : activeTab === 'ngos' ? (
            <NgoCommunitiesView />
            ...
            <>
              {!supabase && <p className="text-xs font-semibold">Demo incident data: Supabase is not configured. Routes are simulations.</p>}
