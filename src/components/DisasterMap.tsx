...
export const DisasterMap: React.FC<DisasterMapProps> = ({
  ...
  return (
    // Full-height stack: the tactical map stretches to fill whatever vertical space the
    // situation room gives it, and the green-zone routing module keeps its natural height
    // beneath. min-h floors keep the map usable when the viewport is short.
    <div className="flex-1 flex flex-col gap-2 min-w-0">
      {/* Map leads the panel: the tactical view is the top of the stack, and the
          green-zone routing module sits beneath it. */}
      <div className="relative w-full flex-1 min-h-[450px] lg:min-h-[560px] bg-sky-100 rounded-xl overflow-hidden border border-sky-100/90 shadow-[0_1px_2px_rgba(8,47,73,0.04),0_14px_30px_-18px_rgba(2,132,199,0.35)] flex flex-col">
      ...
      <button
        ...
        className="absolute top-14 left-3 z-[1000] bg-white/95 dark:bg-sky-900/90 hover:bg-sky-50 dark:hover:bg-sky-800 text-sky-800 dark:text-sky-200 p-2 rounded-lg border border-sky-200 dark:border-sky-700 shadow-[0_4px_12px_-6px_rgba(2,132,199,0.35)] transition cursor-pointer"
      >
        <Maximize2 className="w-4 h-4" />
      </button>

      {/* Map Legend Overlay */}
      <div className="absolute bottom-3 left-3 z-[1000] bg-white/95 dark:bg-sky-900/90 backdrop-blur-xs text-sky-950 dark:text-sky-100 p-2.5 rounded-xl border border-sky-200/80 dark:border-sky-700 shadow-[0_6px_16px_-8px_rgba(2,132,199,0.3)] text-xs">
        <div className="text-[11px] font-bold uppercase tracking-wider text-sky-700 mb-1.5 flex items-center gap-1">
        ...
        <div className="flex flex-col gap-1 text-[11px]">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-sky-900 dark:text-sky-200">Critical (Red)</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-orange-500 ring-2 ring-orange-200"></span>
            <span className="font-medium text-sky-900 dark:text-sky-200">High Priority (Orange)</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-amber-400 ring-2 ring-amber-200"></span>
            <span className="font-medium text-sky-900 dark:text-sky-200">Low Priority (Yellow)</span>
          </div>
        </div>
      </div>
