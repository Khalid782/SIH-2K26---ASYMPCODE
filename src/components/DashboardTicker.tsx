...
export const DashboardTicker: React.FC<DashboardTickerProps> = ({ incidents }) => {
  ...
  return (
    <div className="rounded-xl bg-paper dark:bg-ink/80 backdrop-blur border border-paper-2 dark:border-paper/25 overflow-hidden shadow-[0_8px_24px_-16px_rgba(17,17,16,0.4)] flex items-stretch">
      {/* Feed chip */}
      <div className="shrink-0 flex items-center gap-1.5 px-3.5 py-2.5 text-[10px] font-bold uppercase tracking-widest bg-paper-2/70 text-ink dark:text-paper">
        Feed
      </div>
      ...
      <div className="relative flex-1 overflow-hidden">
        {items.length === 0 ? (
          <div className="py-2.5 px-4 text-[11px] font-medium text-ink dark:text-paper flex items-center h-full">
            Awaiting incoming reports…
          </div>
        ) : (
          <div className="animate-marquee flex whitespace-nowrap py-2.5 text-[11px] font-medium text-ink dark:text-paper">
            {[0, 1].map((dup) => (
              <div key={dup} className="flex shrink-0 items-center">
                {items.map((i) => (
                  <span key={`${dup}-${i.id}`} className="flex items-center gap-1.5 px-4">
                    ...
                    <span className="text-ink dark:text-paper">{i.disasterType}</span>
                    <span className="font-mono text-[10px] text-ink dark:text-paper uppercase">
                      {i.severity} &middot; {i.aiConfidence}%
                    </span>
                    <Radio className="w-2.5 h-2.5 text-ink/40 dark:text-paper/40 mx-2" />
                  </span>
                ))}
              </div>
            ))}
