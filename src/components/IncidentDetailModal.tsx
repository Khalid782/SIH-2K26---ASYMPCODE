      ...
      >
        ...
        <div className="px-5 py-4 bg-gradient-to-r from-paper-2 via-paper to-paper-2/80 text-ink flex items-center justify-between border-b border-paper-2">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-xl bg-paper border border-paper-2 text-mute shadow-[inset_0_1px_0_rgba(255,255,255,0.9),0_4px_10px_-5px_rgba(17,17,16,0.35)]">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-paper-2/80 text-paper-2 border border-paper-2">
                  {incident.status}
                </span>
              </div>
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-mute hover:text-paper hover:bg-paper-2 transition cursor-pointer"
        ...
        <div className="p-5 overflow-y-auto space-y-4 text-xs text-paper/80 leading-relaxed">
          ...
          <div className="p-3.5 rounded-xl bg-paper-2/60 border border-paper-2">
            <div className="flex items-center justify-between text-slate-500 font-semibold mb-1.5">
              ...
              <span className="font-mono text-[11px] text-mute/80 flex items-center gap-1">
          ...
          <div className="border border-paper-2 rounded-xl p-3.5 bg-paper-2/40">
            <div className="text-[11px] font-bold uppercase tracking-wider text-paper-2 mb-3 flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="bg-paper p-3 rounded-lg border border-paper-2/90 shadow-[0_1px_3px_rgba(17,17,16,0.08)]">
                <div className="text-[10px] uppercase font-bold text-mute/80 flex items-center gap-1">
                ...
                {incident.landmark && (
                  <div className="text-[11px] text-mute/70 mt-0.5">
                    {incident.landmark}
                  </div>
                )}
              </div>
              ...
              <div className="bg-paper p-3 rounded-lg border border-paper-2/90 shadow-[0_1px_3px_rgba(17,17,16,0.08)]">
                <div className="text-[10px] uppercase font-bold text-mute/80 flex items-center gap-1">
                  <AlertTriangle className="w-3 h-3 text-mute" />
                  Severity
                </div>
              ...
              <div className="bg-paper p-3 rounded-lg border border-paper-2/90 shadow-[0_1px_3px_rgba(17,17,16,0.08)]">
                <div className="text-[10px] uppercase font-bold text-mute/80 flex items-center gap-1">
                  <Sparkles className="w-3 h-3 text-mute" />
                  AI Confidence
                </div>
                <div className="font-bold text-mute text-xs mt-1 flex items-center gap-1.5">
              ...
              <div className="bg-paper p-3 rounded-lg border border-paper-2/90 shadow-[0_1px_3px_rgba(17,17,16,0.08)]">
                <div className="text-[10px] uppercase font-bold text-mute/80 flex items-center gap-1">
                  <Radio className="w-3 h-3 text-mute" />
                  Source
                </div>
                <div className="font-bold text-ink text-xs mt-1">
                  {incident.source}
              ...
              <div className="bg-paper p-3 rounded-lg border border-paper-2/90 shadow-[0_1px_3px_rgba(17,17,16,0.08)]">
                <div className="text-[10px] uppercase font-bold text-mute/80 flex items-center gap-1">
                  <CheckCircle2 className="w-3 h-3 text-mute" />
                  Verification Status
                </div>
                <div className="font-bold text-ink text-xs mt-1">
                  {incident.status}
              ...
              <div className="bg-paper p-3 rounded-lg border border-paper-2/90 shadow-[0_1px_3px_rgba(17,17,16,0.08)]">
                <div className="text-[10px] uppercase font-bold text-mute/80 flex items-center gap-1">
                  <Compass className="w-3 h-3 text-mute" />
                  Latitude / Longitude
                </div>
              ...
              <div className="mt-3 pt-3 border-t border-paper-2 grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px]">
                {incident.engineUsed && (
                  <div className="sm:col-span-2 flex items-center justify-between bg-paper-2 p-2 rounded-lg border border-paper-2">
                    ...
                    <span className="font-bold text-mute">{incident.engineUsed}</span>
                  </div>
                )}
                {incident.secondaryLocations && incident.secondaryLocations.length > 0 && (
                  <div className="sm:col-span-2 bg-paper-2/70 p-2 rounded-lg border border-paper-2 text-paper/80">
                ...
                {incident.responseNeeded && incident.responseNeeded.length > 0 && (
                  <div className="sm:col-span-2 bg-paper-2/80 p-2 rounded-lg border border-paper-2">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-paper block mb-1">
                      Response Units Needed:
                    </span>
                    <div className="flex flex-wrap gap-1.5">
                      {incident.responseNeeded.map((resp, idx) => (
                        <span key={idx} className="bg-paper text-paper-2 px-2 py-0.5 rounded text-[11px] font-semibold border border-paper-2">
                          {resp}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="sm:col-span-2 bg-paper-2/60 p-2.5 rounded-lg border border-paper-2">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {incident.detectedSignals.map((sig, idx) => (
                        <span key={idx} className="bg-paper text-paper px-2 py-0.5 rounded text-[11px] font-medium border border-paper-2">
                          {sig}
                        </span>
                      ))}
                    </div>
                ...
                {incident.recommendedPriority && (
                  <div className="flex items-center gap-1.5 text-paper font-semibold bg-paper-2 p-2 rounded-lg border border-paper-2">
                    <Sparkles className="w-3.5 h-3.5 text-mute shrink-0" />
                ...
                {incident.entitiesExtracted.waterLevel && (
                  <div className="flex items-center gap-1.5 text-paper-2 font-semibold bg-paper-2 p-2 rounded-lg border border-paper-2">
        ...
        <div className="p-4 bg-gradient-to-r from-white to-paper-2/80 border-t border-paper-2 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="text-xs text-paper-2 font-semibold w-full sm:w-auto">
            Operator Actions:
          </div>
            ...
            <button
              ...
              className={`flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-md font-semibold text-xs transition cursor-pointer ${
                incident.status === 'False Alarm'
                  ...
                  : 'bg-paper hover:bg-paper-2 text-paper-2 border border-paper-2 shadow-xs'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-md font-semibold text-xs transition cursor-pointer ${
                incident.status === 'Duplicate'
                  ...
                  : 'bg-paper hover:bg-slate-50 text-slate-600 border border-slate-200 shadow-xs'
              }`}
            >
